...
file_con = sprintf("/project/results/me/WES/plots/alleleFreq/genotyper/correlation_data/AF_%s_FFex_FFPEex_%s_%s_con_correlation",SAMPLE[s],threshold,targeted);
file_dis = sprintf("/project/results/me/WES/plots/alleleFreq/genotyper/correlation_data/AF_%s_FFex_FFPEex_%s_%s_dis_correlation",SAMPLE[s],threshold,targeted);
pdffile =  sprintf("/project/results/me/WES/plots/alleleFreq/genotyper/correlation_plot_1D/%s/WES_%s_AF1D_%s_FFex_FFPEex_%s_condis_correlation.pdf",threshold,targeted,SAMPLE[s],threshold);
...
for (i in 1:len_con){
	it_CON=it_CON+1
	# print maxconplot concordant lines only
	if ( it_CON < maxconplot ) { lines(xlines,stepcon[i,],col="#999999",lwd=50)  }
}
for (i in 1:len_dis){
...
legend ("bottom", c(paste("concordance, n=",it_CON),paste("discordant calls, n=",it_DIS),paste("false positives, n=",it_FP),paste("false negatives, n=",it_FN)), 
 col=c("white","#FF4500","#1E90FF","#9ACD32"),lty=c(1,1),lwd=50,cex=1.9,text.col=textcolor,border = textcolor,bg = "white")
dev.off()
