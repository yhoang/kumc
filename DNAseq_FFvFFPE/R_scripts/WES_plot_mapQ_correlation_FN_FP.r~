...
file1 = sprintf("/project/results/me/WES/SNV_comparison/genotyper/with_intersection/%s/%s_FFex_FFPEex_%s_%s_con",targeted,SAMPLE[s],threshold,targeted);
file2 = sprintf("/project/results/me/WES/SNV_comparison/genotyper/with_intersection/%s/%s_FFex_FFPEex_%s_%s_dis",targeted,SAMPLE[s],threshold,targeted);

table1=read.table(file1,sep="\t",header=T,fill=TRUE);
table2=read.table(file2,sep="\t",header=T,fill=TRUE);

pdffile = sprintf("/project/results/me/WES/plots/mapQ_correlation/WES_%s_%s_FFex_FFPEex_%s_mapQ_FN_FP.pdf",threshold,targeted,SAMPLE[s],threshold);

minQ = 100
maxQ = 0
# concordances
it_con=0
len_con=length(table1[,6])
stepcon=array(0,c(len_con,101))
for (i in 1:len_con){
        it_con=it_con+1
	stepcon[i,1]=table1[i,6]
	diff=table1[i,18]-table1[i,6]
	for (j in 2:101){
...
for (i in 1:len_dis){
	it_dis=it_dis+1
	stepdis[i,1]=round(table2[i,6])
	diff=round(table2[i,18])-round(table2[i,6])
	for (j in 2:101){
...
for (i in 1:len_con){
	...
	printcon=printcon+1
	if (printcon > printcon_max) {break;}
}

for (i in 1:len_dis){
	#if (is.na(table2[i,5])==TRUE) { #False Positives
	if ((table2[i,17]=="het" || table2[i,17]=="hom")) {
		lines(xlines,stepdis[i,],col="#1E90FF",lwd=50)
		it_FP=it_FP+1
	}
	#else if (is.na(table2[i,17])==TRUE) { #False Negatives
	else if (table2[i,17]!="het" || table2[i,17]!="hom") {
		lines(xlines,stepdis[i,],col="#9ACD32",lwd=50)
		it_FN=it_FN+1
	}
}
for (i in 1:len_dis){
	#if (is.na(table2[i,5])==FALSE && is.na(table2[i,17])==FALSE) { # discordant call
	if (table2[i,5]!="" && table2[i,17]!="") { # discordant call
