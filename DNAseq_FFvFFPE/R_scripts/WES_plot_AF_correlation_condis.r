...
file_con = sprintf("/project/results/me/WES/plots/alleleFreq/genotyper/correlation_data/AF_%s_FFex_FFPEex_%s_%scon_correlation",SAMPLE[s],threshold,targeted);
file_dis = sprintf("/project/results/me/WES/plots/alleleFreq/genotyper/correlation_data/AF_%s_FFex_FFPEex_%s_%sdis_correlation",SAMPLE[s],threshold,targeted);
pdffile = sprintf("/project/results/me/WES/plots/alleleFreq/genotyper/correlation_plot_2D/WES_%sAF2D_%s_FFex_FFPEex_%s_condis_correlation.pdf",targeted,SAMPLE[s],threshold);
...
if (s <10) {
	plot(table_con[,4],table_con[,6], type = "p", xlab = sprintf("allele frequency of %s_FF",SAMPLE[s]), ylab = sprintf("allele frequency of %s_FFPE",SAMPLE[s]), pch=3,lwd=30,xlim=c(0,1),ylim=c(0,1)
	 ,col="#999999",cex.axis=2,cex.lab=2.2,cex=2,col.lab=textcolor,yaxt="n",xaxt="n")
	...
	plot(table_con[,4],table_con[,6], type = "p", xlab = sprintf("allele frequency of %s_FF",SAMPLE2[s]), ylab = sprintf("allele frequency of %s_FFPE",SAMPLE2[s]), pch=3,lwd=30,xlim=c(0,1),ylim=c(0,1)
	 ,col="#999999",cex.axis=2,cex.lab=2.2,cex=2,col.lab=textcolor,yaxt="n",xaxt="n")
	axis(1,lty=1,lwd=7,at=(x1_tick),labels=x1_tick,las=0, cex.axis=2,col=textcolor,col.lab=textcolor,col.axis=textcolor)
	axis(2,lty=1,lwd=7,at=(y1_tick),labels=y1_tick,las=0, cex.axis=2,col=textcolor,col.lab=textcolor,col.axis=textcolor)
}
