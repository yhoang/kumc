...
# Conversion rate of SNVs in targeted intersected discordant positions in FF Files
# i.e. A>T 30%, A>G 30%, A>C 40%
# Looking at base call only (not heterozygosity)

print strftime "%Y-%m-%d %H:%M:%S", localtime(time);
print "\n";
print "./WES_get_FF_conversion_from_discordant_basecall_variants.pl \n";
...
for ( my $k = 0; $k <=$#sample; $k++) {
	...
	$title2 = $title2."\t$sample[$k]";
 	my $dis_file = "</project/results/me/WES/SNV_comparison/genotyper/with_intersection/base_call/$sample[$k]_FFex_FFPEex_${p}_exonic_dis";
        ...
        while(<DISCORDANT_FILE>){
	        ...
	        if ($l[0] eq "chr") {next;}
                ...
                my $varFF = $l[3];
                my $varFFPE = $l[14];
                if ($l[4] eq "\"het\"" || $l[4] eq "\"hom\"") {
                        $conversion[$it][0] = $ref;
                        $conversion[$it][1] = $varFF;
                } else {
                        $conversion[$it][0] = $varFFPE;
                        $conversion[$it][1] = $ref;
                }
                ++$it;
        }
