...
# Compares annotated SNVs between FF and FFPE in intersected positions.

print STDERR "./WGS_compare_gatk_var_anno_with_intersection.pl \n";

our @method = ( "q43cov13" );
our @sample = ("CN");

our ($FF, $FFPE, $INTERSECT, $WRITE_CON, $WRITE_DIS, $TITLE);
$TITLE = "chr\tpos\tref\tFF\thet\tsnpQ\tDepth\tQualbyDepth\tRMS mapping quality\tFunc\tGene\tExonicFunc\tAAChange\tLJB_SIFT_Pred\tFFPE\thet\tsnpQ\tDepth\tQualbyDepth\tRMS mapping quality\n";
...
for (my $o = 0; $o <= $#sample; $o++) {
	...
	foreach my $p (@method) {
		...
		$FF = "</project/results/me/WGS/$sample[$o]_FF/genotyper/anno/$sample[$o]_FFgen.bwa.ug.snp_${p}.genome_summary.csv";
		$FFPE = "</project/results/me/WGS/$sample[$o]_FFPE/genotyper/anno/$sample[$o]_FFPEgen.bwa.ug.snp_${p}.genome_summary.csv";
		$WRITE_CON = ">/project/results/me/WGS/SNV_comparison/genotyper/with_intersection/$sample[$o]_FF_FFPE_${p}_con";
		$WRITE_DIS = ">/project/results/me/WGS/SNV_comparison/genotyper/with_intersection/$sample[$o]_FF_FFPE_${p}_dis";
		...
		my (%FF, %FF, %FF_ref, %w_FF, %FFPE, %FFPE, %FFPE_ref, %w_FFPE, %w_FFPEHelp, %con, %dis, %FN, %FP);
		...
		while(<FF_FILE>){
			...
			$FF{$chr}{$pos} = $snp;
			$FF_ref{$chr}{$pos} = $ref;
			$w_FF{$chr}{$pos} = join("\t",$end,$ref,$snp,$het,$cov,$snpfreq,$thousand,$dbSNP,$func,$gene,$exfunc,$AAchange,$sift);	# info
			++$it_FF;
		}
		close(FF_FILE);
			...
			foreach my $pos (sort keys %{$FFPE{$chr}}){
				...
				if  (!exists $dis{$chr}{$pos} && !exists $con{$chr}{$pos} && exists $compareFF{$chr}{$pos}) {
					if ($p eq "q43cov13" || $p eq "both" && $compareFF{$chr}{$pos} > 12) {
						$FP{$chr}{$pos} = join ("\t","\t\t".$compareFF{$chr}{$pos},$w_FFPEHelp{$chr}{$pos});
						++$FP;
