...
# Compares allele frequency of SNVs calculated from intersected or targeted and intersected pileups between FF and FFPE.
# With differentiation concordance and discordance.

print strftime "%Y-%m-%d %H:%M:%S", localtime(time);
print "\n";
print STDERR "./TES_get_allele_frequency_var_anno_correlation.pl\n";
...
foreach my $o (@sample) {
	...
	foreach my $q (@range) {
	foreach my $p (@method) {
        my $CON_FILE = "</project/results/me/TES/SNV_comparison/genotyper/with_intersection/stats/${o}_FF_FFPE_${p}${q}_con.tsv";
	my $DIS_FILE = "</project/results/me/TES/SNV_comparison/genotyper/with_intersection/stats/${o}_FF_FFPE_${p}${q}_dis.tsv";
        my $WRITE_AF_CON = ">/project/results/me/TES/plots/alleleFreq/genotyper/correlation_data/AF_${o}_FF_FFPE_${p}${q}_con_correlation";
        my $WRITE_AF_DIS = ">/project/results/me/TES/plots/alleleFreq/genotyper/correlation_data/AF_${o}_FF_FFPE_${p}${q}_dis_correlation";
