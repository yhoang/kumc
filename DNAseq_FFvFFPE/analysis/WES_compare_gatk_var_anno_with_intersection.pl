...
print STDERR "./WES_compare_gatk_var_anno_with_intersection.pl \n";

our @method = ("q43cov13");
our @sample = ("2474","2561","2640","2685","2938","3050","3356","4079","4191","CN","CT","DN","DT");

our ($FF, $FFPE, $INTERSECT, $WRITE_CON, $WRITE_DIS, $TITLE);
$TITLE = "chr\tpos\tref\tFF_var\thet\tsnpQ\tDepth\tQualbyDepth\tRMS mapping quality\tFunc\tGene\tExonicFunc\tAAChange\tLJB_SIFT_Pred\tFFPE_var\thet\tsnpQ\tDepth\tQualbyDepth\tRMS mapping quality\n";
...
for (my $o = 0; $o <= $#sample; $o++) {
	...
	foreach my $p (@method) {
		...
		$FF = "</project/results/me/WES/$sample[$o]_FFex/genotyper/anno/$sample[$o]_FFex.bwa.ug.snp_${p}.genome_summary.csv";
		$FFPE = "</project/results/me/WES/$sample[$o]_FFPEex/genotyper/anno/$sample[$o]_FFPEex.bwa.ug.snp_${p}.genome_summary.csv";
		$WRITE_CON = ">/project/results/me/WES/SNV_comparison/genotyper/with_intersection/base_call/$sample[$o]_FFex_FFPEex_${p}_con";
		$WRITE_DIS = ">/project/results/me/WES/SNV_comparison/genotyper/with_intersection/base_call/$sample[$o]_FFex_FFPEex_${p}_dis";
